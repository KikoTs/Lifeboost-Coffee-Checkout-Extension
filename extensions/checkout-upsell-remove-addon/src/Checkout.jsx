...
function SubscribeAndSave() {
  ...
  const fetchProductSellingPlans = async () => {
    try {
      ...
      if (data && data.product) {
        ...
        products = data.product.variants.edges
          .map(edge => ({
            ...edge.node, 
            sellingPlans: edge.node.sellingPlanAllocations.nodes.map(plan => plan.sellingPlan.id)}))
      }

      const product = products.find(prod => prod.id === merchandise.id);
      const sellingPlans = product?.sellingPlanAllocations.nodes.map(plan => ({id: plan.sellingPlan.id, name: plan.sellingPlan.name}) ) || [];
  ...
  if (sellPlanOptions.length && merchandise.sellingPlan) {
    return (
      <Select Select
      label="Delivery"
      value={sellPlanOptions.find(sellPlan => sellPlan.id == merchandise.sellingPlan.id)?.id}
      ...
      options={[...sellPlanOptions.map(sellPlan => ({
        value: sellPlan.id,
        label: sellPlan.name,
    ...
    if (sellPlanOptions.length && !merchandise.sellingPlan) {
      return (
        <Link appearance="monochrome" accessibilityLabel={`Subscribe to ${merchandise.title} from checkout and save 25%`}
          onPress={async () => {
...
function Remove() {
  ...
  if(attributes.find((attribute) => attribute.key === "Special" && attribute.value === "Offer")){
    return (
      <Link
